...
static void show_hex32(uint32_t value) {
...
int main() {
    ...
    while(1) {
        ...
        pred = rf_classify(67, 33, 57, 25);

        after = cycles();
        elapsed = after - before;

        // Mostra classe prevista.
        // Para essa amostra esperamos classe 2, então LD2.
        show_led_value(1 << (pred & 3), 1000000);

        // Pausa separadora.
        for(volatile uint32_t i = 0; i < 1500000; i++) {
        }

        // Mostra o tempo em ciclos, em hexadecimal.
        show_hex32(elapsed);
