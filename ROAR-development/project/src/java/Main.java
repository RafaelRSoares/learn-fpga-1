...
//        xgboostJsonTreeParser.execute(path + "/trees/gbdt/xgboost_tree_0.json");
        ...
        while (true) {
            ...
            if (exists) {
                ...
                /*
                * Os estimadores já foram treinados externamente e estão
                * armazenados em trees/vehicle.csv.
                *
                * Estes valores são metadados usados pelo ROAR para nomear
                * a pasta de saída.
                */
                settingsCLI.trainingParameters = new TrainingParameters();

                settingsCLI.trainingParameters.estimatorsQuantity = 4;
                settingsCLI.trainingParameters.trainingPercent = 80;
                settingsCLI.trainingParameters.maxDepth = 3;
                settingsCLI.trainingParameters.normalizeMaxValue = 0.0;
                settingsCLI.trainingParameters.normalizeMinValue = 0.0;

                System.out.println(
                    "Dataset carregado com floresta externa: "
                    + filename
                );
            } else {
